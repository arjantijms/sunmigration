...
import org.omnifaces.sun.net.www.ParseUtil;
import org.omnifaces.sun.security.util.Debug;
import org.omnifaces.sun.security.util.LocalizedMessage;
import org.omnifaces.sun.security.util.PolicyUtil;
import org.omnifaces.sun.security.util.PropertyExpander;
import org.omnifaces.sun.security.util.ResourcesMgr;
import org.omnifaces.sun.security.util.SecurityConstants;

/**
 * This class represents a default Policy implementation for the "JavaPolicy" type.
 *
 * Note: For backward compatibility with JAAS 1.0 it loads both java.auth.policy and java.policy.
 * However, it is recommended that java.auth.policy not be used and that java.policy contain all
 * grant entries including those that contain principal-based entries.
 *
 * <p>
 * This object stores the policy for the entire Java runtime, and is the amalgamation of multiple
 * static policy configurations that resides in files. The algorithm for locating the policy file(s)
 * and reading their information into this <code>Policy</code> object is:
 *
 * <ol>
 * <li>Read in and load the default policy file named &lt;JAVA_HOME&gt;/lib/security/default.policy.
 * &lt;JAVA_HOME&gt; refers to the value of the java.home system property, and specifies the
 * directory where the JRE is installed. This policy file grants permissions to the modules loaded
 * by the platform class loader. If the default policy file cannot be loaded, a fatal InternalError
 * is thrown as these permissions are needed in order for the runtime to operate correctly.
 * <li>Loop through the <code>java.security.Security</code> properties, <i>policy.url.1</i>,
 * <i>policy.url.2</i>, ..., <i>policy.url.X</i>" and <i>auth.policy.url.1</i>,
 * <i>auth.policy.url.2</i>, ..., <i>auth.policy.url.X</i>". These properties are set in the Java
 * security properties file, which is located in the file named
 * &lt;JAVA_HOME&gt;/conf/security/java.security. Each property value specifies a <code>URL</code>
 ...
 *
 * If none of these could be loaded, use a builtin static policy equivalent to the
 * conf/security/java.policy file.
 *
 * <li>The <code>java.lang.System</code> property <i>java.security.policy</i> may also be set to a
 * <code>URL</code> pointing to another policy file (which is the case when a user uses the -D
 * switch at runtime). If this property is defined, and its use is allowed by the security property
 * file (the Security property, <i>policy.allowSystemProperty</i> is set to <i>true</i>), also load
 * that policy.
 *
 * <li>The <code>java.lang.System</code> property <i>java.security.auth.policy</i> may also be set
 * to a <code>URL</code> pointing to another policy file (which is the case when a user uses the -D
 * switch at runtime). If this property is defined, and its use is allowed by the security property
 * file (the Security property, <i>policy.allowSystemProperty</i> is set to <i>true</i>), also load
 * that policy.
 *
 ...
 *
 * If the <i>java.security.policy</i> or <i>java.security.auth.policy</i> property is defined using
 * "==" (rather than "="), then load the specified policy file and ignore all other configured
 * policies. Note, that the default.policy file is also loaded, as specified in the first step of
 * the algorithm above. If the specified policy file cannot be loaded, use a builtin static policy
 * equivalent to the default conf/security/java.policy file.
 * </ol>
 *
 * Each policy file consists of one or more grant entries, each of which consists of a number of
 * permission entries.
 *
 * <pre>
 ...
 * </pre>
 *
 * All non-bold items above must appear as is (although case doesn't matter and some are optional,
 * as noted below). principal entries are optional and need not be present. Italicized items
 * represent variable values.
 *
 * <p>
 * A grant entry must begin with the word <code>grant</code>. The
 * <code>signedBy</code>,<code>codeBase</code> and <code>principal</code> name/value pairs are
 * optional. If they are not present, then any signer (including unsigned code) will match, and any
 * codeBase will match. Note that the <i>principalClass</i> may be set to the wildcard value, *,
 * which allows it to match any <code>Principal</code> class. In addition, the <i>principalName</i>
 * may also be set to the wildcard value, *, allowing it to match any <code>Principal</code> name.
 * When setting the <i>principalName</i> to the *, do not surround the * with quotes.
 *
 * <p>
 * A permission entry must begin with the word <code>permission</code>. The word
 * <code><i>Type</i></code> in the template above is a specific permission type, such as
 * <code>java.io.FilePermission</code> or <code>java.lang.RuntimePermission</code>.
 *
 * <p>
 * The "<i>action</i>" is required for many permission types, such as
 * <code>java.io.FilePermission</code> (where it specifies what type of file access that is
 * permitted). It is not required for categories such as <code>java.lang.RuntimePermission</code>
 ...
 *
 * <p>
 * The <code>signedBy</code> name/value pair for a permission entry is optional. If present, it
 * indicates a signed permission. That is, the permission class itself must be signed by the given
 * alias in order for it to be granted. For example, suppose you have the following grant entry:
 *
 * <pre>
 ...
 *
 * <p>
 * Then this permission of type <i>Foo</i> is granted if the <code>Foo.class</code> permission has
 * been signed by the "FooSoft" alias, or if XXX <code>Foo.class</code> is a system class (i.e., is
 * found on the CLASSPATH).
 *
 * <p>
 * Items that appear in an entry must appear in the specified order (<code>permission</code>,
 * <i>Type</i>, "<i>name</i>", and "<i>action</i>"). An entry is terminated with a semicolon.
 *
 * <p>
 * Case is unimportant for the identifiers (<code>permission</code>, <code>signedBy</code>,
 * <code>codeBase</code>, etc.) but is significant for the <i>Type</i> or for any string that is
 ...
 * </pre>
 *
 * This Policy implementation supports special handling of any permission that contains the string,
 * "<b>${{self}}</b>", as part of its target name. When such a permission is evaluated (such as
 * during a security check), <b>${{self}}</b> is replaced with one or more Principal class/name
 * pairs. The exact replacement performed depends upon the contents of the grant clause to which the
 * permission belongs.
 * <p>
 *
 * If the grant clause does not contain any principal information, the permission will be ignored
 * (permissions containing <b>${{self}}</b> in their target names are only valid in the context of a
 * principal-based grant clause). For example, BarPermission will always be ignored in the following
 * grant clause:
 *
 * <pre>
 ...
 * </pre>
 *
 * If the grant clause contains principal information, <b>${{self}}</b> will be replaced with that
 * same principal information. For example, <b>${{self}}</b> in BarPermission will be replaced by
 ...
 * </pre>
 *
 * If there is a comma-separated list of principals in the grant clause, then <b>${{self}}</b> will
 * be replaced by the same comma-separated list or principals. In the case where both the principal
 * class and name are wildcarded in the grant clause, <b>${{self}}</b> is replaced with all the
 * principals associated with the <code>Subject</code> in the current
 * <code>AccessControlContext</code>.
 *
 * <p>
 * For PrivateCredentialPermissions, you can also use "<b>self</b>" instead of "<b>${{self}}</b>".
 * However the use of "<b>self</b>" is deprecated in favour of "<b>${{self}}</b>".
 *
...
public class PolicyFile extends java.security.Policy {
    ...
    /**
     * When a policy file has a syntax error, the exception code may generate another permission check
     * and this can cause the policy file to be parsed repeatedly, leading to a StackOverflowError or
     * ClassCircularityError. To avoid this, this set is populated with policy files that have been
     * previously parsed and have syntax errors, so that they can be subsequently ignored.
     */
    private static AtomicReference<Set<URL>> badPolicyURLs = new AtomicReference<>(new HashSet<>());

    // The default.policy file
    private static final URL DEFAULT_POLICY_URL = AccessController.doPrivileged(new PrivilegedAction<URL>() {
        @Override
        public URL run() {
            try {
                return Paths.get(System.getProperty("java.home"), "lib", "security", "default.policy").toUri().toURL();
    ...
    /**
     * Initializes the Policy object and reads the default policy configuration file(s) into the Policy
     * object.
     */
    public PolicyFile() {
        init((URL) null);
     ...
     * object.
     *
     * See the class description for details on the algorithm used to initialize the Policy object.
     */
    ...
    private void initPolicyFile(final PolicyInfo newInfo, final URL url) {
        ...
        if (url != null) {
            ...
            /**
             * If the caller specified a URL via Policy.getInstance, we only read from default.policy and that
             * URL.
             */
            ...
            /**
             * Caller did not specify URL via Policy.getInstance. Read from URLs listed in the java.security
             * properties file.
             *
             * We call initPolicyFile with POLICY, POLICY_URL and then call it with AUTH_POLICY and
             * AUTH_POLICY_URL. So first we will process the JAVA standard policy and then process the JAVA AUTH
             * Policy. This is for backward compatibility as well as to handle cases where the user has a single
             * unified policyfile with both java policy entries and auth entries
             */
    ...
    private boolean init(URL policy, PolicyInfo newInfo, boolean defPolicy) {
        ...
        try (InputStreamReader isr = getInputStreamReader(PolicyUtil.getInputStream(policy))) {
            ...
            try {
                keyStore = PolicyUtil.getKeyStore(policy, pp.getKeyStoreUrl(), pp.getKeyStoreType(), pp.getKeyStoreProvider(),
            ...
            System.err.println(LocalizedMessage.getMessage(POLICY + ".error.parsing.policy.message", source));
            if (debug != null) {
                pe.printStackTrace();
            }
        } catch (Exception e) {
    ...
    private InputStreamReader getInputStreamReader(InputStream is) throws IOException {
        /*
         * Read in policy using UTF-8 by default.
         *
         * Check non-standard system property to see if the default encoding should be used instead.
         */
        return (notUtf8) ? new InputStreamReader(is) : new InputStreamReader(is, "UTF-8");
    }
        ...
        try {
            ...
            while (enum_.hasMoreElements()) {
                ...
                try {
                    ...
                    System.err.println(LocalizedMessage.getMessage(POLICY + ".error.adding.Permission.perm.message", source));
                } catch (Exception e) {
            ...
            System.err.println(LocalizedMessage.getMessage(POLICY + ".error.adding.Entry.message", source));
        }
        if (debug != null)
            debug.println();
    }
    ...
    /**
     * Returns a new Permission object of the given Type. The Permission is created by getting the Class
     * object using the <code>Class.forName</code> method, and using the reflection API to invoke the
     * (String name, String actions) constructor on the object.
     *
     ...
     *
     * @exception InstantiationException if getInstance tries to instantiate an abstract class or an
     ...
     *
     * @exception InvocationTargetException if the underlying Permission constructor throws an
     * exception.
     *
     */
    ...
    /**
     * Creates one of the well-known permissions in the java.base module directly instead of via
     * reflection. Keep list short to not penalize permissions from other modules.
     */
    ...
    /**
     * Creates one of the well-known principals in the java.base module directly instead of via
     * reflection. Keep list short to not penalize principals from other modules.
     */
    ...
    /**
     * Examines this <code>Policy</code> and returns the permissions granted to the specified
     * <code>ProtectionDomain</code>. This includes the permissions currently associated with the domain
     * as well as the policy permissions granted to the domain's CodeSource, ClassLoader, and
     * Principals.
     *
     * <p>
     ...
     * <code>PrivateCredentialPermission</code> which specifies "self" as the <code>Principal</code>
     * class and name, it does not add that <code>Permission</code> to the returned
     * <code>PermissionCollection</code>. Instead, it builds a new
     * <code>PrivateCredentialPermission</code> for each <code>Principal</code> associated with the
     * provided <code>Subject</code>. Each new <code>PrivateCredentialPermission</code> contains the
     * same Credential class as specified in the originally granted permission, as well as the Class and
     * name for the respective <code>Principal</code>.
     *
     * <p>
     *
...
//    }
    ...
    /**
     * Examines this Policy and creates a PermissionCollection object with the set of permissions for
     * the specified CodeSource.
     *
     * @param codesource the CodeSource associated with the caller. This encapsulates the original
     * location of the code (where the code came from) and the public key(s) of its signer.
     *
    ...
    /**
     * Examines the global policy and returns the provided Permissions object with additional
     ...
     *
     * @param perms the permissions to populate
     * @param cs the codesource associated with the caller. This encapsulates the original location of
     * the code (where the code came from) and the public key(s) of its signer.
     *
     * @return the set of permissions according to the policy.
     */
...
//    private void addPerms2(Permissions perms, Principal[] accPs, PolicyEntry entry) {
//        for (int i = 0; i < entry.permissions.size(); i++) {
//            Permission p = entry.permissions.get(i);
//            if (debug != null) {
//                debug.println("  granting " + p);
...
//     *
//     * @param perms the PermissionCollection where the individual Permissions will be added after
//     * expansion.
//     */
//
//    private void expandSelf2(SelfPermission sp, List<PolicyParser.PrincipalEntry> entryPs, Principal[] pdp, Permissions perms) {
//
//        if (entryPs == null || entryPs.isEmpty()) {
//            // No principals in the grant to substitute
//            if (debug != null) {
//                debug.println("Ignoring permission " + sp.getSelfType() + " with target name (" + sp.getSelfName() + ").  "
...
//            perms.add(FilePermCompat.newPermPlusAltPath(getInstance(sp.getSelfType(), sb.toString(), sp.getSelfActions())));
//        } catch (ClassNotFoundException cnfe) {
...
//    }
    ...
    /**
     * return the principal class/name pair in the 2D array. array[x][y]: x corresponds to the array
     * length. if (y == 0), it's the principal class. if (y == 1), it's the principal name.
     */
    ...
    /*
     * Returns the signer certificates from the list of certificates associated with the given code
     * source.
     *
     * The signer certificates are those certificates that were used to verify signed code originating
     * from the codesource location.
     *
     * This method assumes that in the given code source, each signer certificate is followed by its
     * supporting certificate chain (which may be empty), and that the signer certificate and its
     * supporting certificate chain are ordered bottom-to-top (i.e., with the signer certificate first
        ...
        while ((b = pe.name.indexOf("${{", startIndex)) != -1) {
            ...
            if (prefix.equalsIgnoreCase("self")) {
                ...
                if (colonIndex == -1) {
                    ...
                    throw new Exception(LocalizedMessage.getMessage("alias.name.not.provided.pe.name.", source));
                }
                suffix = value.substring(colonIndex + 1);
                if ((suffix = getDN(suffix, keystore)) == null) {
                    Object[] source = { value.substring(colonIndex + 1) };
                    throw new Exception(LocalizedMessage.getMessage("unable.to.perform.substitution.on.alias.suffix", source));
                }
                ...
                throw new Exception(LocalizedMessage.getMessage("substitution.value.prefix.unsupported", source));
            }
        }
     ...
     * <p>
     *
     * A PolicyEntry is a (CodeSource,Permission) pair. The CodeSource contains the (URL, PublicKey)
     * that together identify where the Java bytecodes come from and who (if anyone) signed them. The
     * URL could refer to localhost. The URL could also be null, meaning that this policy entry is given
     * to all comers, as long as they match the signer field. The signer could be null, meaning the code
     * is not signed.
     * <p>
     *
     ...
     * <p>
     *
     * For now, the Policy object retrieves the public key from the X.509 certificate on disk that
     * corresponds to the signedBy alias specified in the Policy config file. For reasons of efficiency,
     * the Policy object keeps a hashtable of certs already read in. This could be replaced by a secure
     * internal key store.
     *
     * <p>
    ...
    private static class PolicyEntry {
         ...
         *
         * @param cs the CodeSource, which encapsulates the URL and the public key attributes from the
         * policy config file. Validity checks are performed on the public key before PolicyEntry is called.
         *
         */
        ...
        /**
         * add a Permission object to this entry. No need to sync add op because perms are added to entry
         * only while entry is being initialized
         */
    ...
    private static class SelfPermission extends Permission {
        ...
        /**
         * Creates a new SelfPermission containing the permission information needed later to expand the
         * self
         *
         * @param type the class name of the Permission class that will be created when this permission is
         * expanded and if necessary resolved.
        ...
        public SelfPermission(String type, String name, String actions, Certificate[] certs) {
            super(type);
            if (type == null) {
                throw new NullPointerException(LocalizedMessage.getMessage("type.can.t.be.null"));
            }
            this.type = type;
            this.name = name;
        ...
        /**
         * This method always returns false for SelfPermission permissions. That is, an SelfPermission never
         * considered to imply another permission.
         *
         ...
         *
         * Checks that <i>obj</i> is an SelfPermission, and has the same type (class) name, permission name,
         * actions, and certificates as this object.
         *
         * @param obj the object we are testing for equality with this object.
         *
         * @return true if obj is an SelfPermission, and has the same type (class) name, permission name,
         * actions, and certificates as this object.
         */
        @Override
         ...
         * "", since there are no actions for an SelfPermission. That is, the actions for the permission
         * that will be created when this SelfPermission is resolved may be non-null, but an SelfPermission
        ...
        /**
         * Returns a string describing this SelfPermission. The convention is to specify the class name, the
         * permission name, and the actions, in the following format: '(unresolved "ClassName" "name"
         * "actions")'.
         *
