 ...
 * principals) is represented as a separate persistent configuration. The configuration may be
 * stored as a flat ASCII file, as a serialized binary file of the Policy class, or as a database.
 *
 * <p>
 * The Java runtime creates one global Policy object, which is used to represent the static policy
 * configuration file. It is consulted by a ProtectionDomain when the protection domain initializes
 * its set of permissions.
 *
 * <p>
 * The Policy <code>init</code> method parses the policy configuration file, and then populates the
 * Policy object. The Policy object is agnostic in that it is not involved in making policy
 * decisions. It is merely the Java runtime representation of the persistent policy configuration
 * file.
 *
 * <p>
 * When a protection domain needs to initialize its set of permissions, it executes code such as the
 * following to ask the global Policy object to populate a Permissions object with the appropriate
 * permissions:
 *
 * <pre>
 ...
 *
 * <p>
 * The protection domain contains a CodeSource object, which encapsulates its codebase (URL) and
 * public key attributes. It also contains the principals associated with the domain. The Policy
 * object evaluates the global policy in light of who the principal is and what the code source is
 * and returns an appropriate Permissions object.
 *
...
public class PolicyParser {
    ...
    public void read(Reader policy) throws ParsingException, IOException {
        ...
        /**
         * Configure the stream tokenizer: Recognize strings between "..." Don't convert words to lowercase
         * Recognize both C-style and C++-style comments Treat end-of-line as white space, not as a token
         */
        ...
        /**
         * The main parsing loop. The loop is executed once for each entry in the config file. The entries
         * are delimited by semicolons. Once we've read in the information for an entry, go ahead and try to
        ...
        if (keyStoreUrlString == null && storePassURL != null) {
            throw new ParsingException(
                    LocalizedMessage.getMessage("keystorePasswordURL.can.not.be.specified.without.also.specifying.keystore"));
        }
    }
    ...
    /**
     * Enumerate all the entries in the global policy object. This method is used by policy admin tools.
     * The tools should use the Enumeration methods on the returned object to fetch the elements
     * sequentially.
     */
    ...
    private void parseKeyStoreEntry() throws ParsingException, IOException {
        ...
        if (peek("\"")) {
            ...
            throw new ParsingException(st.lineno(), LocalizedMessage.getMessage("expected.keystore.type"));
        }
        ...
        if (peek("\"")) {
            ...
            throw new ParsingException(st.lineno(), LocalizedMessage.getMessage("expected.keystore.provider"));
        }
    }
    ...
    private GrantEntry parseGrantEntry() throws ParsingException, IOException {
        ...
        while (!peek("{")) {
            ...
            if (peekAndMatch("Codebase")) {
                if (e.codeBase != null)
                    throw new ParsingException(st.lineno(), LocalizedMessage.getMessage("multiple.Codebase.expressions"));
                e.codeBase = match("quoted string");
                peekAndMatch(",");
            } else if (peekAndMatch("SignedBy")) {
                if (e.signedBy != null)
                    throw new ParsingException(st.lineno(), LocalizedMessage.getMessage("multiple.SignedBy.expressions"));
                ...
                if (actr <= cctr)
                    throw new ParsingException(st.lineno(), LocalizedMessage.getMessage("SignedBy.has.empty.alias"));
                ...
                if (peek("\"")) {
                    ...
                    if (principalClass.equals(PrincipalEntry.WILDCARD_CLASS) && !principalName.equals(PrincipalEntry.WILDCARD_NAME)) {
                        ...
                        throw new ParsingException(st.lineno(),
                                LocalizedMessage.getMessage("can.not.specify.Principal.with.a.wildcard.class.without.a.wildcard.name"));
                    }
                }
                ...
                throw new ParsingException(st.lineno(), LocalizedMessage.getMessage("expected.codeBase.or.SignedBy.or.Principal"));
            }
        }
        ...
        while (!peek("}")) {
            if (peek("Permission")) {
                ...
                throw new ParsingException(st.lineno(), LocalizedMessage.getMessage("expected.permission.entry"));
            }
        }
        match("}");
    ...
    private String match(String expect) throws ParsingException, IOException {
        ...
        case StreamTokenizer.TT_NUMBER:
            throw new ParsingException(st.lineno(), expect, LocalizedMessage.getMessage("number.") + String.valueOf(st.nval));
    ...
    private void skipEntry() throws ParsingException, IOException {
        while (lookahead != ';') {
            switch (lookahead) {
            case StreamTokenizer.TT_NUMBER:
                throw new ParsingException(st.lineno(), ";", LocalizedMessage.getMessage("number.") + String.valueOf(st.nval));
            case StreamTokenizer.TT_EOF:
                throw new ParsingException(LocalizedMessage.getMessage("expected.read.end.of.file."));
            default:
                lookahead = st.nextToken();
            }
        }
    ...
    public static class PrincipalEntry implements Principal {
         ...
         * @throws NullPointerException if principalClass or principalName are null
         */
        public PrincipalEntry(String principalClass, String principalName) {
            if (principalClass == null || principalName == null)
                throw new NullPointerException(LocalizedMessage.getMessage("null.principalClass.or.principalName"));
        ...
        /**
         * Test for equality between the specified object and this object. Two PrincipalEntries are equal if
         * their class and name values are equal.
         *
    ...
    /**
     * Each permission entry in the policy configuration file is represented by a PermissionEntry
     * object.
     *
     * <p>
     * For example, the entry
    ...
    public static class PermissionEntry {
        ...
        /**
         * Calculates a hash code value for the object. Objects which are equal will also have the same
         * hashcode.
         */
        @Override
        public int hashCode() {
    ...
    /**
     * Each domain entry in the keystore domain configuration file is represented by a DomainEntry
     * object.
     */
    ...
    /**
     * Each keystore entry in the keystore domain configuration file is represented by a KeyStoreEntry
     * object.
     */
    ...
    public static class ParsingException extends GeneralSecurityException {
        ...
        /**
         * Constructs a ParsingException with the specified detail message. A detail message is a String
         * that describes this particular exception, which may, for example, specify which algorithm is not
         * available.
         *
