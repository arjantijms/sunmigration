    ...
    private static byte[] convertToBytes(char[] pass) {
        if (enc == null) {
            synchronized (Password.class) {
                ...
                // Uses Native Java_java_io_Console_encoding method to get the console
                // charset on Windows. On Unix (all other operating systems) it's always
                // the default charset.
                // TODO add the Java_java_io_Console_encoding JNI method

                enc = Charset.defaultCharset().newEncoder().onMalformedInput(CodingErrorAction.REPLACE)
